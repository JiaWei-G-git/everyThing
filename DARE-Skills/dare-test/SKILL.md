...
- [ ] **被测源代码文件**：核心被测模块的完整代码（必需）
- [ ] **现有测试代码**：当前测试文件列表和代码（必需）
- [ ] **覆盖率报告**：最近一次测试覆盖率结果（推荐，如 `coverage.json` / `lcov`）
- [ ] **测试计划和验收标准**：测试范围定义文档（推荐）
- [ ] **已知生产事故记录**：历史故障场景（可选，有助于生成边界条件和故障模拟）

**特别注意：** 无覆盖率数据时，审查将重点依赖静态代码分析发现测试盲区。
